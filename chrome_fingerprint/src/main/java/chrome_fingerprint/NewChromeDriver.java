...
public static final Random random = new Random();
	...
	private static volatile String finggerPrintJsData = null;
	...
	public NewChromeDriver(NewChromeOptions options) {
		...
		manage().timeouts().pageLoadTimeout(Duration.ofSeconds(30));//页面加载超时
		...
		if (options.hideFingerprint) {
			...
			//隐藏浏览器指纹
			if (finggerPrintJsData == null) {
				finggerPrintJsData = readFingerPrintJs();
			}
			String fingerPrint = finggerPrintJsData;
			if (fingerPrint != null) {
				parameters.put("source", fingerPrint);
				executeCdpCommand("Page.addScriptToEvaluateOnNewDocument", parameters);
			}
		}
	...
	//运行前再加载浏览器指纹项，主要使用使用js闭包的原理修改值
	//webgl图像指纹、canvas画布指纹、音频指纹、canvas字体指纹、css字体指纹
	public static String readFingerPrintJs() {
		String classPathJs = Settings.fingerPrintJsPath;
		try {
			URL fileURL = NewChromeDriver.class.getResource(classPathJs); 
			byte[] data = IOUtils.readInputStream(fileURL.openStream());
			return new String(data);
