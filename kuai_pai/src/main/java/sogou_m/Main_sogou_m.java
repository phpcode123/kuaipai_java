		...
		while(true) {
			...
			for(String searchKeyword : keywordList) {
				...
				try {
					...
					WebDriver driver =  DriverGet.get(Settings.searchEngineSogouMUrl,proxyIpString);
					...
					try {
						...
						//首页查找和点击
						driver.findElement(By.id("keyword")).sendKeys(searchKeyword);
						OtherClass.timeSleep(1);
						driver.findElement(By.className("qbtn")).click();
						...
						while(baiduPageNum < Settings.maxBaiduPageNum) {
							...
							if(findDomainInHtml == false) {
								...
								OtherClass.timeSleep(1);
								MoveScroll.moveScrollBar(2700*baiduPageNum, driver);
							...
							if(findDomainInHtml == true) {
								...
								while(countNum < 11) {
									...
									String xpathStr = "//div[@data-v=\""+String.valueOf(baiduPageNum*100+countNum)+"\"]";
									...
									try {
										...
										try {
											...
											if(countNum > blockNum) {
												String newXpath = "//div[@data-v=\""+String.valueOf(baiduPageNum*100+countNum)+"\"]";
											...
											if(matchSearchWebsite.size() > 0) {
												...
												try {
													//xpath
													String clickXpath = "//div[@data-v=\""+String.valueOf(baiduPageNum*100+countNum)+"\"]/h3";
							...
							try {
								...
								WebElement findBaiduNextPageLinkurl = driver.findElement(By.id("ajax_next_page"));//注意，这个链接文本一定要与可视化页面的一样，非html代码
								OtherClass.timeSleep(2);
