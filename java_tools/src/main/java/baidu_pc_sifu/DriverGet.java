...
public class DriverGet extends ChromeOptions {
	...
	public static WebDriver get(String url,String proxyString) {
		...
		File webrtcCrxFile = new File(Settings.webrtcControlFilePath);
		...
		extendsFileList.add(webrtcCrxFile);
	    ...
	    try {
		    driver.manage().timeouts().setScriptTimeout(Duration.ofSeconds(35));//脚本执行超时
		    driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(30));//页面加载超时
		    driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5)); //设置匹配等待时间，如果在5s内匹配不到结果就返回错误
	    }catch(Exception e) {
	    	e.printStackTrace();
