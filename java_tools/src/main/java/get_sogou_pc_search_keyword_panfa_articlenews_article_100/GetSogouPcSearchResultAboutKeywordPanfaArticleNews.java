...
public class GetSogouPcSearchResultAboutKeywordPanfaArticleNews {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		while(true) {
			...
			//int itemidNum = OtherClass.getRandomBetweenNumbers(1, 11990);
			
			
			String getSql;
			//getSql = "select * from tp_articlenews_article_100 where search_keyword_status=0 order by itemid asc limit 1;";
			if(lastItemid == 0) {
				getSql = "select * from tp_articlenews_article_100 where search_keyword_status=0 and itemid > "+lastItemid+" order by itemid asc limit 1;";
			}else {
				getSql = "select * from tp_articlenews_article_100 where search_keyword_status=0 and itemid > "+lastItemid+" order by itemid asc limit 1;";
			}
		    ...
		    try {
		        ...
		        while(rs1.next()) {
		        	try {
			        	...
			        	String searchKeywordStatus = rs1.getString(7);
						...
						try {
							...
							if(aboutSearchKeywordString.length() > 5) {
								String updateSql = "update tp_articlenews_article_100 set search_keyword_about_search_keyword=\""+aboutSearchKeywordString+"\",search_keyword_status=1 where itemid="+itemid+";";
								System.out.println(">> updateSql:"+updateSql);
