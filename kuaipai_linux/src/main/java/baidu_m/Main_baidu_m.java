		...
		while(true) {
			...
			for(String searchKeyword : keywordList) {
				...
				try {
					...
					WebDriver driver =  DriverGet.get(Settings.searchEngineBaiduMUrl,proxyIpString);
...
//						OtherClass.timeSleep(10000);
						...
						while(baiduPageNum < Settings.maxBaiduPageNum) {
							...
							//等待页面渲染5秒钟,在页面中可以查看到网址，但是在源码中却查不到，可能是要经过ajax请求渲染。实测暂停5秒后有效。
							System.out.println(">> Timesleep(5), To loading html page source.");
							...
							//有时候会自动跳转到百度app下载页面，如果跳转就再打开一次当前页面
							if(OtherClass.getMatcher("开发者: 百度在线网络技术（北京）有限公司", htmlcode)) {
								System.out.println(">> Download app page.");
								driver.get(baiduSearchLinkurl);
							...
							//打印htmlcode
							//OtherClass.fileWriter("/kuaipai/baidu_m/baidu_htmlcode_log.txt", htmlcode, false);
							//OtherClass.timeSleep(30000);
							...
							if(findDomainInHtml == false) {
								...
								OtherClass.timeSleep(1);
								MoveScroll.moveScrollBar(2200, driver);
							...
							if(findDomainInHtml == true) {
								...
								//================================  设置站长平台点击率开始 ================================
								int clickStatusNum = Settings.clickUrlNumArray[OtherClass.randomNum(Settings.clickUrlNumArray.length)];
								System.out.println(">> clickStatusNum: "+String.valueOf(clickStatusNum));
								
								//当值为0时就不执行点击。
								if(clickStatusNum == 0){
									//匹配查找状态
									baiduPageNum = Settings.maxBaiduPageNum;
									findStatusNum = 1;
									System.out.println(">> clickStatusNum's value is 0, Stop click.");
									break;
								}
								//================================  设置站长平台点击率结束 ================================
								...
								while(countNum < 10) {
									...
									String xpathStr = "//div[@order=\""+String.valueOf(countNum)+"\"]";
									...
									try {
										...
										try {
											...
											//web的窗口小，如果错误就会导致窗口滚动条不下拉，这样后面的运行逻辑都睡是错误。做好错误处理。
											WebElement element;
											String elementText;
											try {
											...
											if(countNum > blockNum) {
												String newXpath = "//div[@order=\""+String.valueOf(countNum)+"\"]";
											...
											if(matchSearchWebsite.size() > 0) {
												...
												try {
													//xpath
													String clickXpath = "//div[@order=\""+String.valueOf(countNum)+"\"]/div[1]/article/header/div/div/h3";
							...
							if(baiduPageNum == 1) {
								nextPageXpath = "//a[@class=\"new-nextpage-only\"]";
							}else {
								nextPageXpath = "//a[@class=\"new-nextpage\"]";
							}
							...
							try {
								...
								WebElement findBaiduNextPageLinkurl = driver.findElement(By.xpath(nextPageXpath));//注意，这个链接文本一定要与可视化页面的一样，非html代码
								//直接获取下一页标签的中的href，下次循环时直接再打开此网页一次，避免跳出百度APP下载页面
