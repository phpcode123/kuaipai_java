...
public class RunAppPc {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		if (scan.hasNext()) {
            RunAppPc.searchWebsite = scan.next();
...
//			
			...
			String keywordFilePath = RunAppPc.keywordFilePath+"pc_"+RunAppPc.searchWebsite+"_keyword.txt";
			...
			for(String searchKeyword : keywordList) {
				try {
					...
					NewChromeOptions.CHROME_DRIVER = "D:\\com.java.selenium_pc\\chromedriver_win32\\chromedriver.exe";
					...
					try {
						...
						int baiduPageNum = 0;
						int countNum = 0;
						while(baiduPageNum < RunAppPc.maxBaiduPageNum) {
							//baiduPageNum count
							baiduPageNum += 1;
										...
										if(findStatusNum == 1) {
											try {
												...
												if(allPageLinkurlList.size() > 0) {
													...
													int randMaxOpenUrlNum = OtherClass.randomNum(RunAppPc.maxOpenUrlNum);
...
//													}
													...
													while(openNum < randMaxOpenUrlNum) {
														...
														//挡开当前页面后随机时间暂停
														int openUrlSleepTime_ = OtherClass.getRandomBetweenNumbers(RunAppPc.openUrlMinSleepTime, RunAppPc.openUrlMaxSleepTime);
														//
														int countNum02 = 0;
														//移动浏览器的滚动条假装浏览刷新页面
