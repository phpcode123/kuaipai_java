...
public class RunApp {
	...
	//windows
	public static String keywordFilePath = "D:\\com.java.selenium_m\\";
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		if (scan.hasNext()) {
            RunApp.searchWebsite = scan.next();
		...
		while(true) {
			...
			//keyword.txt
			String keywordFilePath = RunApp.keywordFilePath+"m_"+RunApp.searchWebsite+"_keyword.txt";
			...
			for(String searchKeyword : keywordList) {
				...
				NewChromeOptions.CHROME_DRIVER = "D:\\com.java.selenium_m\\chromedriver_win32\\chromedriver.exe";
				...
				try {
					...
					int baiduPageNum = 0;
					int countNum = 0;
					while(baiduPageNum < RunApp.maxBaiduPageNum) {
						//baiduPageNum count
						baiduPageNum += 1;
									...
									if(findStatusNum == 1) {
										try {
											...
											if(allPageLinkurlList.size() > 0) {
												...
												int randMaxOpenUrlNum = OtherClass.randomNum(RunApp.maxOpenUrlNum);
...
//												}
												...
												while(openNum < randMaxOpenUrlNum) {
													...
													//挡开当前页面后随机时间暂停
													int openUrlSleepTime_ = OtherClass.getRandomBetweenNumbers(RunApp.openUrlMinSleepTime, RunApp.openUrlMaxSleepTime);
													//
													int countNum02 = 0;
													//移动浏览器的滚动条假装浏览刷新页面
